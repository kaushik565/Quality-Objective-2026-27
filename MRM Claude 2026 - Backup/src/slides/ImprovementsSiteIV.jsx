...
// Departments will be filled once confirmed for this site
const DEPTS = ['— Dept TBD —']

export default function ImprovementsSiteIV() {
  return (
    <SlideShell accentColor={C} sectionLabel="Improvements" slideTitle={`${SITE} — Improvements & Collaborative Meetings`} slideNumber={`${SLIDE_NUM} / 21`}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, height:'100%' }}>
        ...
        <div style={{ border:'1px solid #e5e7eb', borderRadius:4, overflow:'hidden', display:'flex', flexDirection:'column' }}>
          <div style={{ background:'#111111', padding:'8px 14px', flexShrink:0 }}>
              ...
              <tbody>
                {DEPTS.map((dept, i) => (
                  <tr key={dept} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                    <td style={td({ textAlign:'left', fontWeight:700, color:'#6b7280', fontStyle:'italic' })}>{dept}</td>
                    <td style={td({ color:C, fontWeight:800, fontSize:18 })}>—</td>
                  </tr>
                ))}
                <tr style={{ background:'#f3f4f6', borderTop:'2px solid #d1d5db' }}>
                  <td style={td({ textAlign:'left', fontWeight:900, color:'#111111' })}>TOTAL</td>
                  <td style={td({ color:'#CC0000', fontWeight:900, fontSize:20 })}>—</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div style={{ border:'1px solid #e5e7eb', borderRadius:4, overflow:'hidden', display:'flex', flexDirection:'column' }}>
          <div style={{ background:'#CC0000', padding:'8px 14px', flexShrink:0 }}>
            <p style={{ color:'#fff', fontSize:15, fontWeight:800, textTransform:'uppercase', letterSpacing:'0.07em' }}>
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}>
              <thead>
                <tr>
                  ...
                  <th style={th({ background:'#7F1D1D', color:'#fff', width:'32%' })}>Regular Meetings</th>
                  <th style={th({ background:'#CC0000', color:'#fff', width:'32%' })}>Improvement Meetings</th>
                </tr>
              </thead>
              <tbody>
                {DEPTS.map((dept, i) => (
                  <tr key={dept} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                    <td style={td({ textAlign:'left', fontWeight:700, color:'#6b7280', fontStyle:'italic' })}>{dept}</td>
                    <td style={td({ color:'#7F1D1D', fontWeight:700, fontSize:16 })}>—</td>
                    <td style={td({ color:'#CC0000', fontWeight:700, fontSize:16 })}>—</td>
                  </tr>
                ))}
                <tr style={{ background:'#f3f4f6', borderTop:'2px solid #d1d5db' }}>
                  <td style={td({ textAlign:'left', fontWeight:900, color:'#111111' })}>TOTAL</td>
                  <td style={td({ color:'#7F1D1D', fontWeight:900, fontSize:20 })}>—</td>
                  <td style={td({ color:'#CC0000', fontWeight:900, fontSize:20 })}>—</td>
                </tr>
              </tbody>
            </table>
