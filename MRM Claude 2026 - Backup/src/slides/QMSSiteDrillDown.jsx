...
export default function QMSSiteDrillDown() {
  return (
    <SlideShell accentColor="#CC0000" sectionLabel="QMS" slideTitle="Incidents & Deviations — Period Comparison" slideNumber="4 / 21">
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}>
            <thead>
              <tr>
                ...
                <th style={th({ background:'#7F1D1D', color:'#fff', width:'32%' })}>Jun 2025 – Nov 2025</th>
                <th style={th({ background:'#CC0000', color:'#fff', width:'33%' })}>Dec 2025 – May 2026</th>
              </tr>
            </thead>
            <tbody>
              {INCIDENT_ROWS.map((row, i) => (
                <tr key={row} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                  <td style={td({ textAlign:'left', fontWeight:700, color:'#111111' })}>{row}</td>
                  <td style={td({ color:'#7F1D1D', fontWeight:700, fontSize:18 })}>—</td>
                  <td style={td({ color:'#CC0000', fontWeight:700, fontSize:18 })}>—</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {['Site-I','Site-III','Site-IV','Site-V'].map((site, i) => (
                <tr key={site} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                  <td style={td({ textAlign:'left', fontWeight:700, color:'#111111' })}>{site}</td>
                  <td style={td({ color:'#7F1D1D', fontWeight:700, fontSize:18 })}>—</td>
                  <td style={td({ color:'#CC0000', fontWeight:700, fontSize:18 })}>—</td>
                </tr>
              ))}
