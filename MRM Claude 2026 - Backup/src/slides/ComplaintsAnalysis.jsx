      ...
      <div style={{ display:'flex', flexDirection:'column', gap:12, height:'100%' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:12, flex:1, minHeight:0 }}>
              ...
              <tbody>
                {[1,2,3,4,5].map(n=>(
                  <tr key={n} style={{ background:n%2===0?'#f9f9f9':'#fff' }}>
                    <td style={{ color:'#CC0000',fontWeight:800,padding:'14px 20px',fontSize:15,
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {[['Avg Resolution Days','#CC0000'],['Repeat Complaints','#7F1D1D'],['CAPA Linked','#374151']].map(([label,color])=>(
              <div key={label} style={{ border:'1px solid #e5e7eb', borderTop:`3px solid ${color}`,
                ...
                <p style={{ color:'#111111', fontWeight:900, fontSize:28, marginTop:4 }}>—</p>
              </div>
            ))}
          </div>
        </div>
